...
@section('content')
<div class="container">
    <h1 class="my-4 text-center text-primary">Editar Atención</h1>

    <form action="{{ route('atenciones.update', $atencion->id) }}" method="POST" class="bg-light p-4 rounded shadow-sm">
        @csrf
        @method('PUT')
                    ...
                    <select class="form-control" name="dentista_id" id="dentista_id">
                        ...
                        @foreach($dentistas as $dentista)
                            <option value="{{ $dentista->id }}" 
                                {{ $dentista->id == $atencion->dentista_id ? 'selected' : '' }}>
                                {{ $dentista->nombre }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="form-group">
            ...
            <input type="date" class="form-control" name="fecha" id="fecha" required value="{{ $atencion->fecha }}">
        </div>
            ...
            <select class="form-control" name="servicio_id" id="servicio_id" onchange="updateServiceDetails()">
                ...
                @foreach($servicios as $servicio)
                    <option value="{{ $servicio->id }}" 
                            ...
                            {{ $servicio->id == $atencion->servicio_id ? 'selected' : '' }}>
                        {{ $servicio->nombre }}
                    </option>
                @endforeach
            </select>
        ...
        <button type="submit" class="btn btn-success btn-block mt-3">Actualizar Atención</button>
    </form>
</div>
